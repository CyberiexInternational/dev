...
try:
    ...
    file = open("/home/file2")
    gpsdata=file.read()
    file.close()
    logging.info(gpsdata)
    gpsdict=ast.literal_eval(gpsdata[2:len(gpsdata)-1])
    lat=gpsdict["latitude"]
    long=gpsdict["longitude"] 
    ...
    while True:
        ...
        if i==4:
            ...
            gpsdata=file.read()
            file.close()
            gpsddict=ast.literal_eval(gpsdata[2:len(gpsdata)-1])
            lat=gpsdict["latitude"]
            long=gpsdict["longitude"]
            i=0
        ...
        draw.text((tlx+5, tly + 45), "Lat: " + str(lat) + "  Long: " + str(long), fill = "#99ff99",font=Font2)                 
